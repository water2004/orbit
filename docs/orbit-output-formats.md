...
入选包递归所有权树内实际迁移的全部实例内状态文件；`scope = "external"` 的所有权记录和
对应绝对路径不计入统计，也不写入目标。两个子命令
...
| `resolution` | 在命令对应的 Pareto 选择点中选择（`add`/`fix` 为变更极小，`upgrade`/`outdated` 为版本极大；任一命令都可能对独立的包状态或版本因子依次发送多个 interaction，但不会展开完整笛卡尔积）；`data` 只含包动作，`data.changes[].different=true` 是不依赖颜色的差异标记，`kind=keep` 明确表示该方案不执行另一方案中的动作；`selected_artifact` 只含所选顶层 JAR basename，不含目录 |
