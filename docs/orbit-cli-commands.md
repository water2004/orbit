...
也不会引入本可避免的新依赖。包状态与版本候选都按完整约束并图拆成安全因子；安装/缺席、
互斥包、版本及同版本不同 JAR 使用同一种状态模型。CLI 逐因子选择，不展开它们的笛卡尔积，
最后只对选定组合做一次完整求解验证。

该流程会分别显示：递归发现 project、候选队列总数、JAR 下载/缓存校验/解析完成数、
离线求解的动态工作量，以及确认后的包物化进度。Core 将 continuation、冲突核心分支和
maximality probe 累计为单调快照；最多每 100ms 或每 512 个内部事件发布一次，并显示累计的
决策、传播、回溯、冲突和解数量。这样既能区分网络与求解阶段，也不会把数万次传播塞进
CLI 管道或 GUI 队列。该动态总量不是剩余耗时上界；
...
Pareto 极小 front。求解器从不可解推导中的强制偏好核心分支，而不是对每个包反复做线性
黑盒探测；Orbit 按完整约束超图给独立偏好分量，fork 返回公共决定与独立选择因子。固定
偏好后，版本及具体 JAR 实现的 Pareto front 也按同一并图因式化。CLI 逐因子选择后只做一次
完整组合验证，因此不会展开删包、可选安装与版本候选的乘积。
多个互不支配选择仍进入统一方案交互，不按删除数量加权。
...
远端 project 闭包先形成稳定 artifact 队列，缓存查询、下载、校验和候选解析受全局并发上限
约束。求解完成后的最终 JAR 发布保持确定顺序：这一阶段只提交已经验证的选中内容，串行写盘
可以避免同名目标竞态与扩大部分提交面，不是“缺少下载并发”。旧文档把“大规模物化并发”
列为待实现项，混淆了并行准备和确定性提交，现已纠正。
...
- 包身份、Pareto 方案、迁移软解、包事务、cache 清理等交互取消都进入强类型
  `OrbitError::Cancelled`，机器错误码固定为 `cancelled`，进程退出码固定为 3；不解析错误
  文本，也不再把拒绝写入伪装成成功的空事务。
