...
全部由 CLI/core 完成。面板同时区分普通 remove、reset 与 purge：reset 与 purge 都直接启动同一个 Orbit
进程，由 `data_deletion` interaction 展示运行时观测到的准确文件/目录树及逻辑包；GUI 不先
弹一层泛化确认，也不显示配置名称猜测。reset 的动作明确标为“保留包”，只删除归属数据；
purge 明确标为“删除包”，先清理 JAR/TOML/lock 再按同一递归计划删除数据。目录树条目还要
列出其中将保留的其它当前所有者节点。
...
客户端实例页的窗口分辨率使用预设与宽高数值控件，并只调用 Launcher 的强类型
`instance resolution set|clear`；服务端不显示该区域，也不从 GUI 窗口状态推断游戏设置。
