...
> 更新日期：2026-09-22。本文区分“正确规范曾未被代码执行”和“文档本身已经过时”。
...
| 运行时数据归属、reset 与 purge | ✅ | `orbit launch` 单向包装 Launcher 并注入 Runtime Agent；启动前移除账本中已不存在的显式节点；精确 code-source 哈希把顶层/嵌套 JAR 归入逻辑包，snapshot/账本持久化稳定 mod_id，换 JAR 不丢归属；只观测变更不观测读取；文件归最后编辑者，无主目录由首个写入包认领，但共享游戏/世界根保持无主；声明依赖库代写时只沿依赖边回溯调用包，普通写入不走栈扫描；目录默认值按实际文件多数动态重压缩；`ownership` 只读显示压缩数据树，reset 保留包并事务删除数据，purge 再移除包状态；external 仅参与本机清理、不进入导出迁移；无文件名或静态兜底 |
...
本轮修正两类差异：实例物理端本来就是正确规范，但联网候选求解曾误用 `both`，现已与
本地校验统一；“由 Orbit 对全部候选依赖并图分区”则是过时实现，已移除并改为 fork 原生
候选域传播、Pareto 恒定状态证明与剩余约束分区。缺席也成为真正参与冲突回退的状态。
真实 26.2-Fabric 实例只读验证中，BlueMap 安装/不安装已是独立因子，Sodium 候选无删除项。
当前远端数据下 12,096 个组合以 8 组共 34 个局部选项表示；此数量是验证快照，不是固定承诺。
...
- PubGrub fork 已发布到 `water2004/pubgrub` 的 `codex/solver-observer` 分支；
  Orbit 固定到 `0d05cfb3d37382f032e7d2cbb83fda58ec911fd3`。
- 当前 fork 原生支持 `P = mod_id`、不透明复合候选版本、调用方定义
  ...
  推导提取冲突核心、只分支核心成员并剪除已知极小删除集的超集；包缺席或具体候选是统一
  状态，偏好 front 与版本/实现 front 都按独立约束闭包返回为因子，不展开选项乘积。
  无效版本序回调会在产生错误排除前失败。add/fix 对未满足基线偏好的
...
- JAR cache 只保存全局去重字节并执行 LRU；版本库是独立服务。每个精确
  ...
  检查当前作用域，不存在手工 `fetch` 或一次抓取全部游戏版本的路径。
...
- project 闭包的总工作量事前未知，因此显示当前 locator、已发现 artifact 与耗时。
  ...
  增长，它不构成剩余耗时上界。所有命令的独立包状态与版本权衡都按因子交互，不展开完整方案乘积。候选 JAR
...
- [orbit-compatibility.md](orbit-compatibility.md)
