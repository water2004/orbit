...
与单包极大解枚举均由 fork 原生抽象覆盖，不需要在 Orbit 中做第二次求解。
...
该定义要求证明不存在遗漏的局部极大投影。当前 fork 的 continuation/probe 枚举在最坏
情况下仍是指数级；动态工作量只说明求解器正在检查新的投影，不是完成时间上界。尤其
不能把“run 数持续增长”当作终止性证明。有限候选集与每轮确实排除当前版本等价类保证
逻辑终止，但并不保证实际可接受的耗时。
