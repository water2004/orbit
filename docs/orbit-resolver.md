...
Orbit 只提交规范化约束与投影包，不再自行声明分区。fork 对有限且固定的候选全集求闭包，
将缺席纳入候选域，传播已固定状态，并以剩余不兼容子句的超边分区。不可选版本或已经满足的
依赖不再把消费者粘在一起；非投影模块、嵌套 owner、provides 等中间节点仍参与连通性。
版本 front 枚举前还会证明哪些包状态在整个 Pareto front 上恒定：寻找不同状态的可行证据，
最大化后用支配区域剪枝；只有证明恒定才固定该状态并重新分区，不靠抽样或两两独立性猜测。
这一步只探测固定后确实可能拆开剩余图的分隔包，并优先处理能拆出更多组件的包；跳过
没有拆分收益的证明不会固定任何状态，也不会删除候选。有限候选闭包与原始子句只编译一次，
分区与剪枝证明复用同一份模型。

支配查询同时表达“存在集合相同、所有投影版本不下降、至少一个严格上升”，不再逐个包
创建新的求解器。一次连续上升中约束只收紧，冲突学习与依赖缓存得以复用；不同分支之间
不复用带有临时假设的学习结果。上一方案只作为分支优先顺序，不改变更新目标或合法解集合。

恒定性证明中若一个候选被升级回参考状态，fork 会构造可验证的局部替换：从一次严格升级
出发沿原始子句补全必要的内部变量变化，将未改变的投影边界条件作为保护项；被替换的
坐标覆盖整个不高于新版本的范围，其中至少一个坐标严格更低。这个范围内的任何可行方案
都能执行同一替换得到严格更优方案，故可整片剪除。内部模块、具体 JAR 身份与无关包的
独立组合不能成为逐个枚举的理由。等优先级候选与安装/不安装的不可比状态仍保留。

真正耦合、无法证明可拆的组件仍整体枚举，不能承诺任意依赖图都有多项式复杂度。
`add`、`fix`、`constraint set`、
...
求解进度区分普通搜索、联合支配查询、分隔状态证明和偏好保留。查询内的包选择只增加
累计决策数，不覆盖外层查询阶段；增量查询的 observer 保留已有推导，失败查询恢复检查点。
决策次数不是方案数量；`solutions` 是已经保留的局部 Pareto 点数，也不是完整直积数量。
分隔证明期间可以已经找到许多可行证据而 `solutions` 仍为零，不能据此判断尚未找到可行解。
...
与 Pareto 枚举、分区证明均由 fork 原生抽象覆盖；Orbit 只负责构图、展示因子并提交选择。

缺席是参与推导、冲突传播与回退的正式决定，不能在“没有正项待选”时直接把所有未决定包
当成缺席并返回成功。版本 Pareto 序只比较相同投影包存在集合，安装/不安装互不可比；精确
排除和支配区域也必须记录缺席，不能遗漏可选分支或放过“至少安装一个”的不合法组合。

已安装内容在建图时用 `lock:sha512:<digest>`（或 SHA-256）保持选择优先级，下载目录中的
同一字节内容使用不带 `lock:` 的身份。两者必须落入同一个 `same_version` 等价类；它们只是
同一 JAR 的两个输入表示，建图时须去重并保留已安装身份，不能重复注册嵌套 owner 图，
也不能产生“保留/重新安装同一内容”的用户方案。只有内容哈希不同的
...
所有候选求解及本地运行时校验都以 `[platform].physical_environment` 选择物理端；不能
用 `both` 代替实例的端。客户端包含内置服务器，但 Fabric Loader 的物理端仍是 `client`；
`server` 指独立服务端。模块声明和 TOML 包 `env` 是该物理端下的过滤条件，不是另一个
运行时。安装恢复未指定 `--target` 时也遵循实例物理端；显式目标导出/预览仍按请求的端过滤。
服务端专用的嵌套 provider 在客户端不可用，但在独立服务端仍可合法满足依赖。
例如 Voxy 的 server-only `sodiumprovider` 提供 `sodium=0.0.0`，不能使客户端的 Iris
`sodium 0.9.x` 依赖失效。该规则已对照 Fabric Loader 0.19.5 的 ModDiscoverer、Solver 与
EnvType 核对，不以某个模组名称硬编码判断。
