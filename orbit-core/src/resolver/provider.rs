...
pub struct OrbitDependencyProvider {
    ...
    pub versions: HashMap<PackageId, Vec<NormalizedVersion>>,
    /// (package, version) → 前置依赖列表
    pub dependencies: HashMap<(PackageId, NormalizedVersion), Vec<(PackageId, Range<NormalizedVersion>)>>,
    /// (package, version) → 解析后的完整 Mod 数据
    pub resolved_mods: HashMap<(PackageId, NormalizedVersion), crate::providers::ResolvedMod>,
}
...
impl OrbitDependencyProvider {
    ...
    pub fn add_package_deps(
        ...
        deps: Vec<(PackageId, Range<NormalizedVersion>)>,
    ) {
...
impl DependencyProvider<PackageId, NormalizedVersion> for OrbitDependencyProvider {
    fn choose_package_version<T: std::borrow::Borrow<PackageId>, U: std::borrow::Borrow<Range<NormalizedVersion>>>(
        &self,
        potential_packages: impl Iterator<Item = (T, U)>,
    ) -> Result<(T, Option<NormalizedVersion>), Box<dyn Error>> {
    ...
    fn get_dependencies(
        ...
        package: &PackageId,
        version: &NormalizedVersion,
    ) -> Result<Dependencies<PackageId, NormalizedVersion>, Box<dyn Error>> {
