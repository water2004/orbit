...
pub(crate) fn discover_server_runtime(
    ...
    candidates.extend(discover_fabric_bootstraps(instance_dir)?);
    candidates.extend(discover_direct_launch_jars(instance_dir)?);
    candidates.extend(discover_forge_shims(instance_dir)?);
    candidates.extend(discover_modlauncher_argfiles(instance_dir)?);
    merge_equivalent_specs(&mut candidates);
...
fn discover_fabric_bootstraps(instance_dir: &Path) -> Result<Vec<ServerRuntimeSpec>, OrbitError> {
    let mut candidates = Vec::new();
...
fn discover_direct_launch_jars(instance_dir: &Path) -> Result<Vec<ServerRuntimeSpec>, OrbitError> {
    let mut candidates = Vec::new();
    for path in direct_jars(instance_dir)? {
        let Some(manifest) = read_manifest(&path)? else {
            continue;
...
fn discover_forge_shims(instance_dir: &Path) -> Result<Vec<ServerRuntimeSpec>, OrbitError> {
    let mut candidates = Vec::new();
    for path in direct_jars(instance_dir)? {
        let Some(manifest) = read_manifest(&path)? else {
            continue;
...
fn discover_modlauncher_argfiles(
    instance_dir: &Path,
...
fn merge_equivalent_specs(specs: &mut Vec<ServerRuntimeSpec>) {
    let mut merged = Vec::<ServerRuntimeSpec>::new();
    for spec in specs.drain(..) {
        if let Some(existing) = merged.iter_mut().find(|existing| {
            existing.loader == spec.loader
                && existing.loader_version == spec.loader_version
                && existing.minecraft.id == spec.minecraft.id
                && existing.minecraft_jar == spec.minecraft_jar
                && existing.loader_jar == spec.loader_jar
        }) {
            existing.runtime_jars.extend(spec.runtime_jars);
            normalize_runtime_jars(&mut existing.runtime_jars);
            if !existing.evidence.contains(&spec.evidence) {
                existing.evidence.push_str(" + ");
                existing.evidence.push_str(&spec.evidence);
            }
        } else {
            merged.push(spec);
        }
    ...
    merged.sort_by(|left, right| {
        left.loader
            .cmp(&right.loader)
            .then_with(|| left.loader_version.cmp(&right.loader_version))
            .then_with(|| left.minecraft.id.cmp(&right.minecraft.id))
            .then_with(|| left.loader_jar.cmp(&right.loader_jar))
    });
    *specs = merged;
}
