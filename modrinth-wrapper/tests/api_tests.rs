...
async fn test_get_project_dependencies() {
    ...
    assert!(
        deps.is_ok(),
        "Failed to fetch project dependencies: {:?}",
        deps.err()
    );
}
...
async fn test_list_versions_with_params() {
    ...
    assert!(
        result.is_ok(),
        "Failed to list versions with params: {:?}",
        result.err()
    );
    let versions = result.unwrap();
    assert!(!versions.is_empty());
    ...
    for v in &versions {
        assert!(
            ...
            "Version {} does not have fabric loader",
            v.id
        );
    }
}
...
async fn test_get_version_from_hash() {
    ...
    assert!(
        result.is_ok(),
        "Failed to get version from hash: {:?}",
        result.err()
    );
}
...
async fn test_get_versions_from_hashes() {
    ...
    assert!(
        result.is_ok(),
        "Failed to get versions from hashes: {:?}",
        result.err()
    );
...
async fn test_get_latest_version_from_hash() {
    ...
    let game_versions: Vec<&str> = versions[0]
        .game_versions
        .iter()
        .map(|s| s.as_str())
        .collect();
    ...
    let result = client
        .get_latest_version_from_hash(hash, &loaders, &game_versions, None)
        .await;
    assert!(
        result.is_ok(),
        "Failed to get latest version from hash: {:?}",
        result.err()
    );
}
...
async fn test_get_latest_versions_from_hashes() {
    ...
    let result = client
        .get_latest_versions_from_hashes(&[hash], &loaders, &game_versions, None)
        .await;
    assert!(
        result.is_ok(),
        "Failed to get latest versions from hashes: {:?}",
        result.err()
    );
    let map = result.unwrap();
    assert!(map.contains_key(hash));
