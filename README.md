...
- **Runtime-owned package data.** `orbit launch` wraps Orbit Launcher and injects Orbit's low-overhead Java Agent. It never observes reads. Exact code-source hashes map top-level and Loader-declared nested JARs to one logical package; a created directory owns its recursive user content, while deeper creators and foreign writes form preserved exceptions. Export, migration, and `purge` all consume that same ledger. There is no filename, stack-walk, or static-analysis fallback.
...
| `orbit purge <package>` | Show one exact package/data deletion plan, including preserved nested ownership, then remove the logical package from TOML/lock and recursively delete its observed ownership roots. |
