...
sudo apt install ./orbit-launcher_0.6.2-1_amd64.deb

# Optional mod package management on that server.
sudo apt install ./orbit_0.6.2-1_amd64.deb
...
sudo apt install ./orbit_0.6.2-1_amd64.deb \
  ./orbit-launcher_0.6.2-1_amd64.deb \
  ./orbit-gui_0.6.2-1_amd64.deb
```
...
consent with `--allow-removals`. Independent package-state and version trade-offs are returned as a
factored product: absence or one concrete JAR candidate is the common atomic state. The CLI asks
once per factor instead of expanding Cartesian combinations, then validates the selected product
once. Every remaining incomparable choice still requires an explicit decision.
