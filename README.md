...
# 3. 搜索并安装模组 (自动匹配当前 MC 版本与 Loader)
orbit install sodium
orbit install cf:jei
...
| `orbit update` | **检查更新（只读）**。联网检查所有已安装模组是否有新版本，并输出更新报告。不修改任何文件。 |
| `orbit upgrade [mod]` | **执行更新**。下载并替换可更新的 `.jar` 文件，并更新 `orbit.toml`。如果不带参数，则升级所有可升级的模组。 |
...
| `orbit install` | 根据当前的 `orbit.toml`，下载并补齐所有缺失的 `.jar` 文件。 |
| `orbit install <mod>` | 安装新模组。支持自动查找，或指定来源（如 `mr:name`, `cf:name`, `file:./my-mod.jar`）。 |
...
每一个被 Orbit 接管的 `.minecraft` 目录下都会生成一个 `orbit.toml` 文件。它是该实例状态的“唯一真实数据源 (Single Source of Truth)”。
...
[dependencies]
# 平台托管模组
sodium = { source = "modrinth", version = "^0.5.8" }
jei = { source = "curseforge" }

# 通过 orbit sync 自动接管的未知/手动拖入模组
journeymap = { source = "modrinth", version = "5.9.8" }

# 本地直连/自制模组
optifine = { source = "file", path = "./downloads/OptiFine.jar" }
```

> **提示**：强烈建议将 `orbit.toml` 纳入 Git 版本控制！结合 `orbit install`，你可以随时随地在不同的电脑上秒速还原完整的模组环境。
