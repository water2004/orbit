...
pub trait ModProvider {
    ...
    /// 获取当前平台的模组分类树
    async fn get_categories(&self) -> Result<Vec<Category>, Box<dyn std::error::Error>>;
    ...
    /// 解析模组的依赖项
    async fn resolve_dependency(&self, id: &str) -> Result<Option<UnifiedMod>, Box<dyn std::error::Error>>;
}
